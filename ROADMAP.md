...
- [x] integration test: configure the mock provider via a generated `models.json` under an isolated `PI_CODING_AGENT_DIR` instead of a `registerProvider` test-fixture extension, so the test drives pi's stock custom-provider config path end-to-end. removed `tests/_mock_provider.ts` and the now-dead `tests/mock_openai.py` symlink.
